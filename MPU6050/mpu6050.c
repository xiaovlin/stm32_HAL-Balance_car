...
/**
  ...
  * 说    明: 无
  */ 
void mpu6050_get_data(int16_t *gx, int16_t *gy, int16_t *gz, 
                      int16_t *ax, int16_t *ay, int16_t *az, 
                      int16_t *temperature)
{
  uint8_t data_buf[14];

  MPU6050_ReadData(MPU6050_BURST_ADDR, &data_buf[0], 14);

  *ax = data_buf[0] * 0x100 + data_buf[1];                  //读取MPU6050的加速度数据
  *ay = data_buf[2] * 0x100 + data_buf[3];                 
  *az = data_buf[4] * 0x100 + data_buf[5];
  *temperature = data_buf[6] * 0x100 + data_buf[7];        //读取MPU6050的原始温度数据
  *gx = data_buf[8] * 0x100 + data_buf[9];
  *gy = data_buf[10] * 0x100 + data_buf[11];
  *gz = data_buf[12] * 0x100 + data_buf[13];               //读取MPU6050的角加速度数据
}
